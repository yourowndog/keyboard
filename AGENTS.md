...
* **Plan:** Eliminate the race where overlapping Whisper uploads cleared the active recording reference, causing subsequent captures to abort during transcription.
* **Changes:** Null out `audioFile` before launching the coroutine and only when it still points to the file being uploaded so concurrent recordings keep their own file handles. Left deletion logic scoped to the captured file.
* **Attempts:** No local builds or tests executed (Android SDK/toolchain unavailable in this environment).
