...
### 2026-07-11 — 2026-07-14 — Correction-system stabilization (Phase 2)
* **Task:** Refactored the autocorrect commit path into distinct layers (Retriever → Judge → Gate → Caser) so that a single Gate decides every automatic commit. Full arc and rationale live in `docs/history/2026-07-12-direction-map.md`; canonical behavior in `docs/autocorrect/live-pipeline.md`.
* **Details:**
  - **Gate extraction:** pulled the commit decision out of the ranker into `ime/nlp/shared/CommitPolicy.kt` (pure object, named veto `Blocker`s) and `ime/nlp/shared/CorrectionDecision.kt`, which assembles typed evidence (`CommitRequestEvidence` / `CommitCandidateEvidence`, `NeuralEvidence`, `ContractionLicense`) and is now the sole commit boundary. Routed the primary path, the casing/licensed-contraction shortcuts, and the fallback through it (`ShortcutCorrection`, `FallbackCorrection` adapters).
  - **Stage 2 fallback provenance:** `SymSpellManager.suggest` now returns structured `FallbackCandidate`s (raw, cased, provenance, real edit distance, `FallbackEngineMode`, optional contraction license) instead of `List<String>`. The SymSpell-only fallback no longer fabricates edit-correction authority; `LEGACY_FALLBACK` was flipped to `hasCorrectionProvenance=false` and cannot auto-commit on rank alone. Intended delta: the 4 unlicensed `LEGACY_FALLBACK_SHORTCUTS` (wheres/theres/hell/shell) now show but no longer auto-commit.
  - **Safeguards preserved & tested:** numeric/identifier protection, the one narrow number-row slip, valid-word immunity, protected vocab, anti-correction, contraction-license binding, neural-bypass-not-evaluated. Asset-backed assembled-pipeline regression suite added. Full debug unit suite green (78/78); debug APK builds and is installed on-device.
  - **Files:** `SymSpellManager.kt`, `latin/LatinLanguageProvider.kt`, `shared/CommitPolicy.kt`, `shared/CorrectionDecision.kt`, `shared/FallbackCorrection.kt` (new), plus `shared/CommitPolicyTest.kt`, `shared/CorrectionDecisionTest.kt`, `shared/FallbackCorrectionTest.kt` (new), and docs.
* **Diagnosed, deferred to bug queue:** autocorrect never fires in `noSuggestions` fields (search boxes, Termux, URL bars) — `AbstractEditorInstance.shouldDetermineComposingRegion` returns false there, so no composing region is established and the NLP layer never sees a word. Confirmed from device harvest; not an engine/refactor regression. See `ROADMAP.md`.
